...
import { mono, italic } from "../common/text.mjs";
...
function runAdvanced(command) {
  const yourCommand = italic(`Your command was ${mono(command)}`);
  try {
    ...
    return {
      message: `${message}. ${yourCommand}.`,
      error: errorMessage != undefined,
    };
  } catch (error) {
    return {
      message: `An unknown error occurred (likely, you tried to discard too many rolls). ${yourCommand}.`,
      error: true,
    };
  }
}
