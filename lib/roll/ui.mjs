...
function runAdvanced(command) {
  try {
    ...
    return {
      message: `${message}. *Your command was \`${command}\`.*`,
      error: errorMessage != undefined,
    };
  } catch (error) {
    return {
      message: `An unknown error occurred (likely, you tried to discard too many rolls). *Your command was \`${command}\`.*`,
      error: true,
    };
  }
}
